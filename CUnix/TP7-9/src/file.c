...
void add_word(char* name, int fileindex, word_list* list) {
    ...
    if(list->first_word == NULL){
        list->first_word = newelt;
        return;
    }

    if(list->first_word != NULL && list->last_word == NULL ){
        list->last_word = newelt;
        list->first_word->next = list->last_word;
        return;
    }

    if (list->last_word != NULL) {
		list->last_word->next = newelt;
        list->last_word = list->last_word->next;
        return;
	}
